...
export const studentFetchQuiz = () => {
    return async (dispatch) => {
        try {
            const { data } = await axios({
                method: 'Get',
                url: url + "/api/student/allQuiz",
            })
            dispatch(StudentGetAllQuiz(data))
        }
        catch (err) {
            dispatch({
                type: SET_ERRORS,
...
export const studentFetchResult = (userId) => {
    return async (dispatch) => {
        try {
            const { data } = await axios({
                method: 'Get',
                url: url + "/api/student/getResult",
                data: userId
            })
            dispatch(StudentgetResult(data))
        }
        catch (err) {
            dispatch({
                type: SET_ERRORS,
